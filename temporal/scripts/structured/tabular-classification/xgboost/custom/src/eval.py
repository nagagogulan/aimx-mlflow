...
# Get XGBoost-specific metrics if available
xgb_metrics = {}
if isinstance(model, xgb.XGBClassifier):
    ...
    for feature, gain in importance_gain.items():
        xgb_metrics[f"gain_{feature}"] = float(gain)
    
    # Get feature importance by weight
    importance_weight = booster.get_score(importance_type='weight')
    for feature, weight in importance_weight.items():
        xgb_metrics[f"weight_{feature}"] = float(weight)
    
    # Get feature importance by cover
    importance_cover = booster.get_score(importance_type='cover')
    for feature, cover in importance_cover.items():
        xgb_metrics[f"cover_{feature}"] = float(cover)
