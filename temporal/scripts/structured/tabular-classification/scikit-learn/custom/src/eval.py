...
# Scikit-learn model specific metrics
# Feature importance
feature_importance = {}
...
if hasattr(model, 'coef_'):
    if len(model.coef_.shape) == 1:
        ...
        for i, coef in enumerate(model.coef_):
            sklearn_metrics[f"coefficient_{X.columns[i]}"] = float(coef)
    else:
        # Multi-class
        for class_idx, class_coef in enumerate(model.coef_):
            for i, coef in enumerate(class_coef):
                sklearn_metrics[f"coefficient_class{class_idx}_{X.columns[i]}"] = float(coef)

# For tree-based models, get additional metrics
...
# Print scikit-learn model specific metrics
if sklearn_metrics:
    print("\nScikit-learn model specific metrics:")
...
with mlflow.start_run():
    ...
    # Log scikit-learn model specific metrics
    for metric_name, value in sklearn_metrics.items():
