...
try:
    ...
    if y_prob is not None:
        if len(y_prob.shape) == 2 and y_prob.shape[1] == 2:
            ...
            # If y_prob is 1D array of probabilities
...
try:
    if y_prob is not None:
        if len(y_prob.shape) == 2 and y_prob.shape[1] == 2:
            # For binary classification with 2 columns of probabilities
...
# Get LightGBM-specific metrics if available
lgb_metrics = {}
if isinstance(model, lgb.LGBMClassifier):
    # Get feature importance by split
    for i, importance in enumerate(model.booster_.feature_importance(importance_type='split')):
        lgb_metrics[f"split_importance_{X.columns[i]}"] = float(importance)
    
    # Get feature importance by gain
    for i, importance in enumerate(model.booster_.feature_importance(importance_type='gain')):
        lgb_metrics[f"gain_importance_{X.columns[i]}"] = float(importance)
    
    # Get number of trees
    lgb_metrics["num_trees"] = model.booster_.num_trees()
