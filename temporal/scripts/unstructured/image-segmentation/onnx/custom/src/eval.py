...
def preprocess_image(image_path, input_size=(224, 224), config=None):
    """Preprocess image for ONNX segmentation model"""
    ...
    # Apply custom preprocessing if specified in config
    if config and 'preprocessing' in config:
        if config['preprocessing'] == 'normalize_0_1':
            # Normalize to [0, 1]
            img_np = img_np / 255.0
        elif config['preprocessing'] == 'normalize_imagenet':
            # Normalize with ImageNet mean and std
            img_np = img_np / 255.0
            mean = np.array([0.485, 0.456, 0.406]).reshape(1, 1, 3)
            std = np.array([0.229, 0.224, 0.225]).reshape(1, 1, 3)
            img_np = (img_np - mean) / std
        elif config['preprocessing'] == 'normalize_minus1_1':
            # Normalize to [-1, 1]
...
# Load ONNX Model
# ----------------------------
print(f"Loading custom ONNX segmentation model from: {weight_path}")
try:
...
for idx, (image_path, mask_path) in enumerate(tqdm(image_mask_pairs)):
    ...
    img_np, original_img, original_size = preprocess_image(image_path, input_size=input_size, config=model_config)
...
if len(image_mask_pairs) > 0:
    ...
    for idx, (image_path, mask_path) in enumerate(image_mask_pairs):
        # Load and preprocess image
        img_np, _, original_size = preprocess_image(image_path, input_size=input_size, config=model_config)
    ...
    conf_matrix = confusion_matrix(all_gts, all_preds, labels=unique_classes)
    ...
    plt.tight_layout()
    plt.savefig('./output_visualizations/confusion_matrix.png')
    plt.close()
...
with mlflow.start_run():
    ...
    mlflow.log_metric("mean_pixel_acc", mean_pixel_acc)
    
    for class_idx in valid_classes:
        mlflow.log_metric(f"{class_names[class_idx]}_iou", class_miou[class_idx])
        mlflow.log_metric(f"{class_names[class_idx]}_dice", class_mdice[class_idx])
    
    # Log confusion matrix image
    mlflow.log_artifact('./output_visualizations/confusion_matrix.png')
