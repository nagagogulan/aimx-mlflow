...
input_size = int(os.getenv("INPUT_SIZE", "224"))  # Default ResNet-50 input size
...
def preprocess_image(image_path, input_size=(224, 224)):
    """Preprocess image for TensorFlow segmentation model with ResNet-50 backbone"""
...
try:
    ...
    print("Attempting to load as a standard TensorFlow segmentation model...")
    
    try:
        # Try loading as a standard TensorFlow segmentation model
        if model_type.lower() == "fcn":
            # Create FCN model with ResNet-50 backbone
            ...
            print("Loaded FCN model with ResNet-50 backbone")
            
        elif model_type.lower() == "deeplabv3":
            # Try to import DeepLabV3 from TensorFlow models
            try:
                ...
                # Create DeepLabV3 model (simplified version)
                x = base_model.output
                ...
                # Load weights
                model.load_weights(weight_path)
                print("Loaded DeepLabV3 model with ResNet-50 backbone")
                
            except ImportError:
                print("DeepLabV3 not available, falling back to FCN model")
                # Fall back to FCN model
                base_model = tf.keras.applications.ResNet50(
                    include_top=False, weights='imagenet', input_shape=(input_size, input_size, 3)
                )
...
if len(image_mask_pairs) > 0:
    ...
    for idx, (image_path, mask_path) in enumerate(image_mask_pairs):
        # Load and preprocess image
        img_np, _, original_size = preprocess_image(image_path, input_size=(input_size, input_size))
