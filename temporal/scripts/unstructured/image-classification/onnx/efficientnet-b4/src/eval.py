...
input_size = int(os.getenv("INPUT_SIZE", "380"))  # EfficientNet-B4 default input size

# ----------------------------
# Helper Functions
# ----------------------------
def preprocess_image(image_path, input_size=(380, 380)):
    """Preprocess image for EfficientNet-B4 ONNX model"""
    image = Image.open(image_path).convert("RGB")
    image = image.resize(input_size)
    image_data = np.array(image).astype(np.float32) / 255.0
    
    # EfficientNet normalization (ImageNet)
...
# Load ONNX Model
# ----------------------------
print(f"Loading ONNX model from: {weight_path}")
try:
...
if mlflowURI:
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("model_type", "EfficientNet-B4-ONNX")
