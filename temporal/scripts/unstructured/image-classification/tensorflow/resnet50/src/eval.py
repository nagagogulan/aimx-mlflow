...
def preprocess_image(image_path, input_size=(224, 224)):
    ...
    return preprocess_input(img_array)  # ResNet-50 specific preprocessing
...
# ----------------------------
print(f"Loading ResNet-50 model from: {weight_path}")
try:
    ...
    # Check if the model has the expected ResNet-50 architecture
    if 'resnet' not in weight_path.lower() and not any('resnet' in layer.name.lower() for layer in model.layers):
        print("Warning: This may not be a ResNet-50 model. Continuing anyway...")
    ...
    print("Attempting to load as a ResNet-50 model from TensorFlow...")
    try:
        # Try loading as a standard ResNet-50 model
        from tensorflow.keras.applications import ResNet50
        model = ResNet50(weights='imagenet')
        print("Loaded default ResNet-50 model with ImageNet weights")
...
if mlflowURI:
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("model_type", "ResNet50-TensorFlow")
